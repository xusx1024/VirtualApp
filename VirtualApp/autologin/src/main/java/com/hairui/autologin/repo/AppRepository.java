...
public class AppRepository implements AppDataSource {
  ...
  private static final List<String> SCAN_PATH_LIST =
      Arrays.asList(".", "wandoujia/app", "tencent/tassistant/apk", "BaiduAsa9103056",
          "360Download", "pp/downloader", "pp/downloader/apk", "pp/downloader/silent/apk");
  ...
  @Override public Promise<List<AppData>, Throwable, Void> getVirtualApps() {
  ...
  @Override public Promise<List<AppInfo>, Throwable, Void> getInstalledApps(Context context) {
    return VUiKit.defer()
        .when(() -> convertPackageInfoToAppData(context,
            context.getPackageManager().getInstalledPackages(0), true));
  }
  ...
  public Promise<List<AppInfo>, Throwable, Void> getStorageApps(Context context, File rootDir) {
    return VUiKit.defer()
        .when(() -> convertPackageInfoToAppData(context,
            findAndParseAPKs(context, rootDir, SCAN_PATH_LIST), false));
  }
  ...
  @Override public InstallResult addVirtualApp(AppInfoLite info) {
  ...
  @Override public boolean removeVirtualApp(String packageName, int userId) {
