...
public class MyAppRequestListener implements VirtualCore.AppRequestListener {
  ...
  @Override public void onRequestInstall(String path) {
    ...
    if (res.isSuccess) {
      ...
      if (res.isUpdate) {
        Toast.makeText(context, "Update: " + res.packageName + " success!", Toast.LENGTH_SHORT)
            .show();
      } else {
        Toast.makeText(context, "Install: " + res.packageName + " success!", Toast.LENGTH_SHORT)
            .show();
      }
    } else {
  ...
  @Override public void onRequestUninstall(String pkg) {
