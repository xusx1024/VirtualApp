...
class HomePresenterImpl implements HomeContract.HomePresenter {
  ...
  @Override public void start() {
    ...
    if (!Once.beenDone(VCommends.TAG_ASK_INSTALL_GMS)
        && GmsSupport.isOutsideGoogleFrameworkExist()) {
      mView.askInstallGms();
      Once.markDone(VCommends.TAG_ASK_INSTALL_GMS);
    }
  }

  @Override public void launchApp(AppData data) {
    try {
      if (data instanceof PackageAppData) {
        ...
        LoadingActivity.launch(mActivity, multipleData.appInfo.packageName,
  ...
  @Override public void addApp(AppInfoLite info) {
    class AddResult {
  ...
  @Override public void deleteApp(AppData data) {
    try {
  ...
  @Override public void createShortcut(AppData data) {
    VirtualCore.OnEmitShortcutListener listener = new VirtualCore.OnEmitShortcutListener() {
      @Override public Bitmap getIcon(Bitmap originIcon) {
        return originIcon;
      }

      @Override public String getName(String originName) {
