...
public class ListAppFragment extends VFragment<ListAppContract.ListAppPresenter>
    implements ListAppContract.ListAppView {
  ...
  @Override public void onViewCreated(View view, Bundle savedInstanceState) {
    ...
    mAdapter.setOnItemClickListener(new CloneAppListAdapter.ItemEventListener() {
      @Override public void onItemClick(AppInfo info, int position) {
        ...
        if (!mAdapter.isIndexSelected(position)) {
          if (count >= 9) {
            Toast.makeText(getContext(), R.string.install_too_much_once_time, Toast.LENGTH_SHORT)
                .show();
            return;
          }
        }
    ...
    mAdapter.setSelectionListener(count -> {
      ...
      mInstallButton.setText(
          String.format(Locale.ENGLISH, getResources().getString(R.string.install_d), count));
    });
  ...
  @Override public void loadFinish(List<AppInfo> infoList) {
  ...
  @Override public void setPresenter(ListAppContract.ListAppPresenter presenter) {
