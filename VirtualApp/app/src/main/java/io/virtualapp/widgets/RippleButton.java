...
@SuppressLint("ClickableViewAccessibility") public class RippleButton extends AppCompatButton {
  ...
  @Override public boolean onTouchEvent(final MotionEvent event) {
    ...
    if (event.getActionMasked() == MotionEvent.ACTION_DOWN && this.isEnabled() && mHover) {
      ...
      if (mAnimationIsCancel =
          !mRect.contains(getLeft() + (int) event.getX(), getTop() + (int) event.getY())) {
        setRadius(0);
      } else {
        setRadius(dp(50));
      }
  ...
  public void setRadius(final float radius) {
    ...
    if (mRadius > 0) {
      mRadialGradient =
          new RadialGradient(mDownX, mDownY, mRadius, adjustAlpha(mRippleColor, mAlphaFactor),
